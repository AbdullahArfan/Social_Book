...
# Model
from Order_System.models import Cart, Order
...
def add_to_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            return redirect("Shop_System:market")
        else:
...
def cart_view(request):
    ...
    if carts.exists() and orders.exists():
        order = orders[0]
        return render(request, 'Order_System/cart.html', context={'carts':carts, 'order':order})
    else:
...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            return redirect("Order_System:cart")
        else:
